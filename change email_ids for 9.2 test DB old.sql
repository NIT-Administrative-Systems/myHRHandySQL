...
-- Update the rcapture parameters so the reports go to the correct folder in VISTA

-- the code below should be 1 for HR92DEVA
--                          2 for HR92DEVB
--                          3 for HR92DEVC
--                          4 for HR92DEVD
--                          Q for HR92QA
...
update PS_NW_BATCH_STEP a
set a.nw_os_cmd_line = (select substr(b.nw_os_cmd_line,1,11)
                            || case (select upper(dbname) from ps.psdbowner)
                                 when 'HR92DEVA' then '1'
                                 when 'HR92DEVB' then '2'
                                 when 'HR92DEVC' then '3'
                                 when 'HR92DEVD' then '4'
                                 when 'HR92QA'   then 'Q'
                                 when 'HR92UPG'  then 'U'
                                 else 'X'
                               end
                            || substr(b.nw_os_cmd_line,12)
                        from PS_NW_BATCH_STEP b
                        where b.NW_JOB_NAME = a.NW_JOB_NAME
                        and b.NW_STEP_NBR = a.NW_STEP_NBR)
where nw_os_cmd_line like '%rcap%';
